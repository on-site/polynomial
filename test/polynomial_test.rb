...
class TestPoly < MiniTest::Unit::TestCase
  ...
  def test_incorrect_initialization
    assert_raises(ArgumentError) { Poly.new }
    assert_raises(ArgumentError) { Poly.new [] }
    assert_raises(TypeError) { Poly.new :oops }
    assert_raises(TypeError) { Poly.new ['foo'] }
  end
  ...
  def test_new_from_coefficients
    ...
    Poly.new [1,2]
    Poly.new [1, 2.2]
    Poly.new [1, Complex(0,1)]
    Poly.new [1, BigDecimal('1e-2')]
    Poly.new 1,2
